...
class Wc_Muse_Cron {
	...
	public function add_cron_schedule() {
		...
		if ( wp_next_scheduled( 'wc_muse_cron_events' ) ) return;
		...
		if ( isset( $schedules[$this->schedule_recurrence] ) ) {
			wp_schedule_event( time(), $this->schedule_recurrence, 'wc_muse_cron_events' );
		}

	}

	public function uneschedule_wc_muse_cron_events() {
		wp_clear_scheduled_hook( 'wc_muse_cron_events' );
	}

	public function on_settings_saved( $old_value, $new_value ) {
		if ( $old_value !== $new_value ) {
			//	If settings value was changed, we need to remove the cron
			$this->uneschedule_wc_muse_cron_events();
		}
	}
